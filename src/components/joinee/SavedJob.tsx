...
export default function SavedJobs() {
    ...
    return (
       <div style={{ fontFamily: "'DM Sans', sans-serif", background: "var(--bg-primary)", minHeight: "100vh", color: "var(--text-primary)" }}>
           ...
           <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 32px", height: 62, borderBottom: "2px solid var(--border-color)", background: "var(--bg-card)", position: "sticky", top: 0, zIndex: 50, boxShadow: "0 2px 12px rgba(13,27,62,0.06)" }}>
            ...
            <main style={{ maxWidth: 1100, margin: "0 auto", padding: "36px 32px 60px" }}>
                <h1 style={{ fontFamily: "'Sora',sans-serif", fontSize: 26, fontWeight: 700, color: "var(--text-primary)", marginBottom: 6 }}>Saved Jobs</h1>
                ...
                {loading ? (
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: 16 }}>
                        {jobs.map((job) => (
                            <div key={job.id} style={{ background: "var(--bg-card)", border: "1.5px solid var(--border-color)", borderRadius: 14, padding: 20, display: "flex", flexDirection: "column", gap: 13, boxShadow: "0 2px 8px rgba(13,27,62,0.04)" }}>
                                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                                    ...
                                    <button onClick={() => handleUnsave(job.id)} title="Remove from saved" style={{ background: "var(--bg-secondary)", border: "1.5px solid var(--border-color)", borderRadius: 8, width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 15 }}>
                                        🔖
                                    </button>
                                </div>

                                <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                                    <span style={{ fontSize: 11, background: "var(--bg-secondary)", color: "var(--accent-color)", border: "1px solid var(--border-color)", padding: "3px 9px", borderRadius: 20, fontWeight: 600 }}>{job.type}</span>
                                    <span style={{ fontSize: 11, color: "var(--text-secondary)" }}>📍 {job.location}</span>
                                </div>

                                <div style={{ display: "flex", gap: 5, flexWrap: "wrap" as const }}>
                                    {(job.tags ?? []).map((tag: string) => (
                                        <span key={tag} style={{ fontSize: 10, background: "var(--bg-secondary)", color: "var(--text-primary)", border: "1px solid var(--border-color)", padding: "2px 8px", borderRadius: 5 }}>{tag}</span>
                                    ))}
                                </div>
