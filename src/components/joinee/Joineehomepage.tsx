...
export default function JoineeHomepage() {
    ...
    useEffect(() => { getProfile().then((p) => setProfile(p)).catch(() => {}); }, []);
    useEffect(() => { getJobStats().then((d) => setStats(d)).catch(() => {}); }, []);
    useEffect(() => { getSavedJobIds().then((ids: number[]) => setSavedIds(new Set(ids))).catch(() => {}); }, []);
    ...
    const fetchJobs = async (pageNum: number, filter: FilterOption, query: string, append = false) => {
        try {
            if (!append) setLoading(true); else setLoadingMore(true);
            const params: Record<string, string> = { status: "open", page: String(pageNum), limit: "12" };
    ...
    const handleSearchChange = (value: string) => {
        ...
        searchTimeout.current = setTimeout(() => { setPage(1); fetchJobs(1, activeFilter, value); }, 300);
    };
    ...
    const handleToggleSave = async (id: number) => {
        ...
        setProfile((p) => p ? { ...p, savedJobs: (p.savedJobs ?? 0) + (isSaved ? -1 : 1) } : p);
        try {
    ...
    const handleApply = async (id: number) => {
        ...
        try {
            ...
            setProfile((p) => p ? { ...p, applications: (p.applications ?? 0) + 1 } : p);
            showToast("Applied successfully ✓  We'll notify you of updates.", "success");
        } catch (err: unknown) {
            const status = (err as { status?: number })?.status;
            if (status === 409) { showToast("You've already applied to this role.", "error"); setAppliedIds((prev) => new Set(prev).add(id)); }
            else                { showToast("Application failed. Please try again.", "error"); }
        }
    };

    const handleLoadMore = () => { const n = page + 1; setPage(n); fetchJobs(n, activeFilter, search, true); };
      ...
      `}</style>
            ...
            <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 32px", height: 62, borderBottom: "2px solid #E8EDF8", background: "#fff", position: "sticky", top: 0, zIndex: 50, boxShadow: "0 2px 12px rgba(13,27,62,0.06)" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
                    {NAV_LINKS.map(({ label, href }) => (
                        <button key={label} onClick={() => navigate(href)} className="nav-link"
                            style={{ color: "#4A5568", fontSize: 13, fontWeight: 500, padding: "6px 14px", borderRadius: 7, cursor: "pointer", transition: "color 0.15s", background: "none", border: "none", fontFamily: "inherit" }}>
                            {label}
                        </button>
                    ))}
                    {/* Job Deck nav link */}
                    <button onClick={() => navigate("/joinee/swipe")} className="nav-link"
                        style={{ color: "#D62B2B", fontSize: 13, fontWeight: 600, padding: "6px 14px", borderRadius: 7, cursor: "pointer", background: "#FFF5F5", border: "1px solid #FECDD3", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 5 }}>
                        🃏 Job Deck
                    </button>
                </div>
                ...
                <div ref={profileRef} style={{ position: "relative" }}>
                    <button onClick={() => setProfileOpen((p) => !p)}
                        ...
                        <svg width="11" height="11" viewBox="0 0 12 12" fill="none"><path d="M2 4l4 4 4-4" stroke="#6B7280" strokeWidth="1.5" strokeLinecap="round" /></svg>
                    </button>
                    ...
                    {profileOpen && (
                        <div style={{ position: "absolute", right: 0, top: "calc(100% + 10px)", width: 256, background: "#fff", border: "1.5px solid #E8EDF8", borderRadius: 16, padding: 6, zIndex: 100, boxShadow: "0 24px 60px rgba(13,27,62,0.14)" }}>
                            <div style={{ padding: "12px 14px", borderBottom: "1px solid #F0F4FF", marginBottom: 4 }}>
                                ...
                                <div style={{ fontSize: 11, color: "#6B7280", marginBottom: 5 }}>Profile {profile?.completionPercent ?? 0}% complete</div>
                            ...
                            {DASHBOARD_LINKS.map(({ icon, label, href }) => (
                                <button key={label} className="menu-item"
            ...
            <section style={{ padding: "60px 32px 40px", textAlign: "center", background: "linear-gradient(160deg,#fff 60%,#F3F5FF 100%)", borderBottom: "1px solid #E8EDF8", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ position: "relative", zIndex: 1 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", maxWidth: 560, margin: "0 auto", background: "#fff", border: "2px solid #E8EDF8", borderRadius: 14, padding: "5px 5px 5px 16px", boxShadow: "0 4px 20px rgba(13,27,62,0.08)" }}>
                        ...
                        <button onClick={() => { if (searchTimeout.current) clearTimeout(searchTimeout.current); setPage(1); fetchJobs(1, activeFilter, search); }}
            ...
            <main style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 32px 60px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                    ...
                    <button onClick={() => navigate("/joinee/swipe")}
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, marginBottom: 24 }}>
                    ...
                    {!loading && <span style={{ fontSize: 12, color: "#6B7280" }}>{jobs.length} result{jobs.length !== 1 ? "s" : ""} found</span>}
                </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: 16 }}>
                        {Array.from({ length: 9 }).map((_, i) => (
                            <div key={i} style={{ background: "#fff", border: "1.5px solid #E8EDF8", borderRadius: 14, padding: 20, display: "flex", flexDirection: "column", gap: 12 }}>
                                ...
                                <div style={{ display: "flex", gap: 5 }}>{[60, 80, 50].map((w, j) => <div key={j} className="skeleton" style={{ height: 20, width: w }} />)}</div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: 16 }}>
                        {jobs.map((job, index) => (
                            <JobCard key={`${job.id}-${index}`} job={job}
                                isSaved={savedIds.has(job.id)} isApplied={appliedIds.has(job.id)}
                                onSave={handleToggleSave} onApply={handleApply} />
                        ))}
                    </div>
                )}
            ...
            {/* Toast */}
            {toast && (
                <div style={{ position: "fixed", bottom: 28, left: "50%", transform: "translateX(-50%)", background: toastType === "error" ? "#D62B2B" : "#0D1B3E", color: "#fff", fontSize: 13, padding: "10px 22px", borderRadius: 30, zIndex: 999, boxShadow: "0 8px 32px rgba(13,27,62,0.25)", whiteSpace: "nowrap", fontFamily: "inherit", borderLeft: `4px solid ${toastType === "error" ? "#fff" : "#D62B2B"}` }}>
                    {toast}
                </div>
            )}
        </div>
...
interface JobCardProps {
    ...
    onSave: (id: number) => void; onApply: (id: number) => void;
}
        ...
        <div className="job-card" style={{ background: "#fff", border: "1.5px solid #E8EDF8", borderRadius: 14, padding: 20, display: "flex", flexDirection: "column", gap: 13, boxShadow: "0 2px 8px rgba(13,27,62,0.04)" }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
                    <div style={{ width: 42, height: 42, borderRadius: 10, background: job.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 700, color: "#fff", flexShrink: 0 }}>{job.logo}</div>
                    <div>
                ...
                <button onClick={() => onSave(job.id)} title={isSaved ? "Unsave" : "Save"}
            ...
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                {job.tags.map((tag, index) => (
                    <span key={`${tag}-${index}`} style={{ fontSize: 10, background: "#F8F9FF", color: "#4A5568", border: "1px solid #E8EDF8", padding: "2px 8px", borderRadius: 5, fontWeight: 500 }}>{tag}</span>
                ))}
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "auto", paddingTop: 10, borderTop: "1px solid #F0F4FF" }}>
                ...
                <button className="apply-btn" onClick={() => onApply(job.id)} disabled={isApplied}
