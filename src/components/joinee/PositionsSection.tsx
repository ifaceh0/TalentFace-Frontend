...
// import { addPosition, updatePosition, deletePosition } from '../../services/joinee.service';
// import type { Position } from '../../types/joinee.types';

// const empty = (): Position => ({ role: '', organization: '', description: '', startDate: '', endDate: '', isOngoing: false });

// interface PositionsSectionProps {
//   items: Position[];
//   onChange: (items: Position[]) => void;
// }

// export default function PositionsSection({ items, onChange }: PositionsSectionProps) {
...
//   const f = (field: keyof Position, val: string | boolean) => setForm((p) => ({ ...p, [field]: val }));
//   const openAdd = () => { setEditing(null); setForm(empty()); setShowModal(true); };
//   const openEdit = (item: Position) => { setEditing(item); setForm({ ...item }); setShowModal(true); };

//   const handleSave = async () => {
//     setLoading(true);
//     try {
//       const updated = editing?._id ? await updatePosition(editing._id, form) : await addPosition(form);
//       onChange(updated);
//       setShowModal(false);
...
//     setDeleting(id);
//     try { onChange(await deletePosition(id)); } catch { /* */ } finally { setDeleting(null); }
//   };
...
//         <div>
//           <h2 className="text-xl font-bold text-gray-900">Positions of Responsibility</h2>
//           <p className="text-sm text-gray-500 mt-0.5">Club roles, student body positions, leadership roles</p>
//         </div>
...
//           <FormField label="Role / Title" value={form.role ?? ''} onChange={(e) => f('role', (e.target as HTMLInputElement).value)} placeholder="President, Secretary…" required />
//           <FormField label="Organization / Club" value={form.organization ?? ''} onChange={(e) => f('organization', (e.target as HTMLInputElement).value)} placeholder="Tech Club, Student Council…" />
...
//           </label>
//           <FormField as="textarea" label="Description" value={form.description ?? ''} onChange={(e) => f('description', (e.target as HTMLTextAreaElement).value)} placeholder="Responsibilities and achievements…" rows={3} />
