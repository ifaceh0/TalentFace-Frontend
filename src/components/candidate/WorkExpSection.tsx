...
import { addWorkExperience, updateWorkExperience, deleteWorkExperience } from '../../services/candidate.service';
import type { WorkExperience } from '../../types/candidate.types';
...
export default function WorkExpSection({ items, onChange }: WorkExpSectionProps) {
  ...
  const f = (field: keyof WorkExperience, val: string | boolean | JSONContent) =>
  ...
  // description is stored as Tiptap JSONContent — convert to HTML so bold/italic/etc
  // still show up in the card preview instead of being flattened to plain text
  const descriptionHTML = (json?: WorkExperience['description']): string => {
    if (!json) return '';
    if (typeof json === 'string') return json;
    try {
      return generateHTML(json as JSONContent, [StarterKit]);
    } catch {
      return '';
    }
  };
            ...
            <div key={exp._id} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  ...
                  {exp.description && (
                    <div
                      className="text-xs text-gray-500 mt-2 line-clamp-2 prose prose-xs max-w-none [&_strong]:font-semibold [&_em]:italic [&_u]:underline [&_s]:line-through"
                      dangerouslySetInnerHTML={{ __html: descriptionHTML(exp.description) }}
                    />
                  )}
                </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <RichTextEditor
              ...
              onChange={(json) => f('description', json)}
              maxChars={1000}
              minHeight={140}
            />
          </div>
