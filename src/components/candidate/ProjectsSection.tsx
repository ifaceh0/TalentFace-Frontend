...
import { addProject, updateProject, deleteProject } from '../../services/candidate.service';
import type { Project } from '../../types/candidate.types';
...
export default function ProjectsSection({ items, onChange }: ProjectsSectionProps) {
  ...
  const f = (field: keyof Project, val: string | boolean | string[] | JSONContent) =>
  ...
  const descriptionHTML = (json?: Project['description']): string => {
    if (!json) return '';
            ...
            <div key={proj._id} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  ...
                  {proj.description && (
                    <div
                      className="text-sm text-gray-600 mt-1 line-clamp-2 prose prose-sm max-w-none [&_strong]:font-semibold [&_em]:italic [&_u]:underline [&_s]:line-through"
                      dangerouslySetInnerHTML={{ __html: descriptionHTML(proj.description) }}
                    />
                  )}
