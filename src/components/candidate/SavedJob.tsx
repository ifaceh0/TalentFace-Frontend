...
import { getSavedJobIds, getSavedJobs, unsaveJob } from "../../services/candidate.service";
...
export default function SavedJobs() {
    ...
    return (
       <div style={{ fontFamily: "'DM Sans', sans-serif", background: "var(--bg-primary)", minHeight: "100vh", color: "#111827" }}>
           ...
           <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 32px", height: 62, borderBottom: "2px solid #E5E7EB", background: "#ffffff", position: "sticky", top: 0, zIndex: 50, boxShadow: "0 2px 12px rgba(13,27,62,0.06)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }} onClick={() => navigate("/candidate/home")}>
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: "linear-gradient(135deg,#D62B2B,#1C3FA8)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: 14, color: "#fff" }}>CAN</div>
                    <span style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: 18, letterSpacing: "-0.5px" }}>candidate</span>
                </div>
            ...
            <main style={{ maxWidth: 1100, margin: "0 auto", padding: "36px 32px 60px" }}>
                <h1 style={{ fontFamily: "'Sora',sans-serif", fontSize: 26, fontWeight: 700, color: "#111827", marginBottom: 6 }}>Saved Jobs</h1>
                ...
                {loading ? (
                    ...
                    <div style={{ textAlign: "center", padding: "80px 0" }}>
                        ...
                        <button onClick={() => navigate("/candidate/browse")} style={{ background: "linear-gradient(135deg,#D62B2B,#1C3FA8)", border: "none", borderRadius: 10, padding: "10px 24px", fontSize: 13, fontWeight: 600, color: "#fff", cursor: "pointer", fontFamily: "inherit" }}>
                            Browse Jobs →
                        </button>
                    </div>
                ) : (
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: 16 }}>
                        {jobs.map((job) => (
                            <div key={job.id} style={{ background: "#ffffff", border: "1.5px solid #E5E7EB", borderRadius: 14, padding: 20, display: "flex", flexDirection: "column", gap: 13, boxShadow: "0 2px 8px rgba(13,27,62,0.04)" }}>
                                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                                    ...
                                    <button onClick={() => handleUnsave(job.id)} title="Remove from saved" style={{ background: "#F8FAFC", border: "1.5px solid #E5E7EB", borderRadius: 8, width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 15 }}>
                                        🔖
                                    </button>
                                </div>

                                <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                                    <span style={{ fontSize: 11, background: "#F8FAFC", color: "var(--accent-color)", border: "1px solid #E5E7EB", padding: "3px 9px", borderRadius: 20, fontWeight: 600 }}>{job.type}</span>
                                    <span style={{ fontSize: 11, color: "#6B7280" }}>📍 {job.location}</span>
                                </div>

                                <div style={{ display: "flex", gap: 5, flexWrap: "wrap" as const }}>
                                    {(job.tags ?? []).map((tag: string) => (
                                        <span key={tag} style={{ fontSize: 10, background: "#F8FAFC", color: "#111827", border: "1px solid #E5E7EB", padding: "2px 8px", borderRadius: 5 }}>{tag}</span>
                                    ))}
                                </div>
                                ...
                                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "auto", paddingTop: 10, borderTop: "1px solid #E5E7EB" }}>
                                    ...
                                    <button onClick={() => navigate(`/candidate/jobs/${job.id}`)} style={{ background: "linear-gradient(135deg,var(--accent-color),var(--primary-color))", border: "none", borderRadius: 9, padding: "9px 20px", fontSize: 12, fontWeight: 600, color: "#fff", cursor: "pointer", fontFamily: "inherit" }}>
                                        View →
                                    </button>
                                </div>
                            </div>
...
//                                         <div style={{ fontSize: 10, color: "#6B7280", marginTop: 2 }}>{job.posted}</div>
//                                     </div>
//                                     <button onClick={() => navigate(`/joinee/jobs/${job.id}`)} style={{ background: "linear-gradient(135deg,var(--accent-color),var(--primary-color))", border: "none", borderRadius: 9, padding: "9px 20px", fontSize: 12, fontWeight: 600, color: "#fff", cursor: "pointer", fontFamily: "inherit" }}>
//                                         View →
//                                     </button>
//                                 </div>
//                             </div>
