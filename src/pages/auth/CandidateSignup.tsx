...
import { signupCandidate as apiSignupCandidate } from "../../services/auth.service";
...
export default function CandidateSignup() {
  const [form, setForm]             = useState<CandidateForm>({ name: "", email: "", password: "", confirmPassword: "", phone: "" });
  const [showPw, setShowPw]         = useState(false);
  const [showCpw, setShowCpw]       = useState(false);
  const [loading, setLoading]       = useState(false);
  const [errors, setErrors]         = useState<Partial<CandidateForm>>({});
  const [apiError, setApiError]     = useState("");
  const [phoneError, setPhoneError] = useState('');
  const navigate                    = useNavigate();
  const { setAuth }                 = useAuth();

  const set = (f: keyof CandidateForm) => (e: React.ChangeEvent<HTMLInputElement>) => {
    setForm((prev) => ({ ...prev, [f]: e.target.value }));
    setErrors((prev) => ({ ...prev, [f]: undefined }));
  };

  const validate = () => {
    const errs: Partial<CandidateForm> = {};
...
setPhoneError('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { token, user } = await apiSignupCandidate({
        name: form.name,
        email: form.email,
  ...
  />
            ...
            <button id="candidate-submit" type="submit" disabled={loading}
