...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const loadProfile = async (): Promise<void> => {
      try {
        ...
        setFormData({
          name: recruiter.name ?? user?.name ?? '',
          email: recruiter.email ?? user?.email ?? '',
          countryCode: recruiter.countryCode ?? '+91',
  ...
  const handlePhoneChange = (
    ...
    if (numbersOnly.length > 10) {
      setPhoneError('Phone number cannot contain more than 10 digits.');
      return;
    }
  ...
  const handleSave = async (): Promise<void> => {
    ...
    try {
      const updatedRecruiter = await updateRecruiterProfile({
        name: formData.name,
      ...
      setFormData({
        ...
        email: updatedRecruiter.email ?? formData.email,
        countryCode: updatedRecruiter.countryCode ?? '+91',
