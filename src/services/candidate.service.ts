...
export const getProfile = async (): Promise<CandidateProfile> => {
  const { data } = await api.get<ApiEnvelope<{ candidate: CandidateProfile }>>('/candidate/profile');
  return data.data.candidate;
};

export const getProfileCompletion = async (): Promise<{ score: number; profileComplete: boolean }> => {
  const { data } = await api.get<ApiEnvelope<{ score: number; profileComplete: boolean }>>('/candidate/profile/completion');
  return data.data;
};

export const updateBasicDetails = async (payload: Partial<CandidateProfile>): Promise<CandidateProfile> => {
  const { data } = await api.patch<ApiEnvelope<{ candidate: CandidateProfile }>>('/candidate/profile/basic', payload);
  return data.data.candidate;
};

export const updateSummary = async (summary: JSONContent): Promise<{ summary: JSONContent }> => {
  const { data } = await api.patch<ApiEnvelope<{ summary: JSONContent }>>('/candidate/profile/summary', { summary });
  return data.data;
};

// export const updateSummary = async (summary: string): Promise<{ summary: string }> => {
//   const { data } = await api.patch<ApiEnvelope<{ summary: string }>>('/candidate/profile/summary', { summary });
//   return data.data;
// };

export const updateAddress = async (address: Address): Promise<{ address: Address }> => {
  const { data } = await api.patch<ApiEnvelope<{ address: Address }>>('/candidate/profile/address', { address });
  return data.data;
};
...
//   const { data } = await api.post<ApiEnvelope<{ profilePhoto: string }>>('/candidate/profile/photo', form, {
...
export const getEducation = async (): Promise<Education[]> => {
  const { data } = await api.get<ApiEnvelope<{ education: Education[] }>>('/candidate/education');
  return data.data.education;
};

export const addEducation = async (payload: Education): Promise<Education[]> => {
  const { data } = await api.post<ApiEnvelope<{ education: Education[] }>>('/candidate/education', payload);
  return data.data.education;
};

export const updateEducation = async (eduId: string, payload: Education): Promise<Education[]> => {
  const { data } = await api.patch<ApiEnvelope<{ education: Education[] }>>(`/candidate/education/${eduId}`, payload);
  return data.data.education;
};

export const deleteEducation = async (eduId: string): Promise<Education[]> => {
  const { data } = await api.delete<ApiEnvelope<{ education: Education[] }>>(`/candidate/education/${eduId}`);
  return data.data.education;
};
export const uploadEducationDocuments = async (
  ...
  const { data } = await api.post<ApiEnvelope<{ education: Education[] }>>(
    `/candidate/education/${eduId}/documents`,
    formData
  );
  return data.data.education;
};
...
export const deleteEducationDocument = async (
  ...
  const { data } = await api.delete<ApiEnvelope<{ education: Education[] }>>(
    `/candidate/education/${eduId}/documents/${field}`
  );
  return data.data.education;
};
...
export const getWorkExperience = async (): Promise<WorkExperience[]> => {
  const { data } = await api.get<ApiEnvelope<{ workExperience: WorkExperience[] }>>('/candidate/work-experience');
  return data.data.workExperience;
};

export const addWorkExperience = async (payload: WorkExperience): Promise<WorkExperience[]> => {
  const { data } = await api.post<ApiEnvelope<{ workExperience: WorkExperience[] }>>('/candidate/work-experience', payload);
  return data.data.workExperience;
};

export const updateWorkExperience = async (expId: string, payload: WorkExperience): Promise<WorkExperience[]> => {
  const { data } = await api.patch<ApiEnvelope<{ workExperience: WorkExperience[] }>>(`/candidate/work-experience/${expId}`, payload);
  return data.data.workExperience;
};

export const deleteWorkExperience = async (expId: string): Promise<WorkExperience[]> => {
  const { data } = await api.delete<ApiEnvelope<{ workExperience: WorkExperience[] }>>(`/candidate/work-experience/${expId}`);
  return data.data.workExperience;
};
...
export const updateSkills = async (skills: string[]): Promise<{ skills: string[] }> => {
  const { data } = await api.patch<ApiEnvelope<{ skills: string[] }>>('/candidate/skills', { skills });
  return data.data;
};
...
export const getProjects = async (): Promise<Project[]> => {
  const { data } = await api.get<ApiEnvelope<{ projects: Project[] }>>('/candidate/projects');
  return data.data.projects;
};

export const addProject = async (payload: Project): Promise<Project[]> => {
  const { data } = await api.post<ApiEnvelope<{ projects: Project[] }>>('/candidate/projects', payload);
  return data.data.projects;
};

export const updateProject = async (projectId: string, payload: Project): Promise<Project[]> => {
  const { data } = await api.patch<ApiEnvelope<{ projects: Project[] }>>(`/candidate/projects/${projectId}`, payload);
  return data.data.projects;
};

export const deleteProject = async (projectId: string): Promise<Project[]> => {
  const { data } = await api.delete<ApiEnvelope<{ projects: Project[] }>>(`/candidate/projects/${projectId}`);
  return data.data.projects;
};
...
export const updateSocialProfiles = async (socialProfiles: SocialProfile[]): Promise<SocialProfile[]> => {
  const { data } = await api.patch<ApiEnvelope<{ socialProfiles: SocialProfile[] }>>('/candidate/social-profiles', { socialProfiles });
...
//   const { data } = await api.post<ApiEnvelope<{ resumeUrl: string }>>('/candidate/resume', form, {
//     headers: { 'Content-Type': 'multipart/form-data' },
//   });
//   return data.data;
// };
...
export const saveJob = async (id: string) => {
    await api.post(`/jobs/${id}/save`);        // ← was /candidate/jobs/
};
...
export const unsaveJob = async (id: string) => {
    await api.delete(`/jobs/${id}/save`);      // ← was /candidate/jobs/
};
...
export const getSavedJobIds = async (): Promise<string[]> => {
    const res = await api.get('/jobs/saved/ids');  // ← was /candidate/jobs/
    return res.data.data.ids;
};
export const getSavedJobs = async () => {
    const res = await api.get('/jobs/saved');      // ← was /candidate/jobs/
...
export const generate2FASecret = async (): Promise<{ secret: string; otpauthUrl: string }> => {
  const { data } = await api.post<ApiEnvelope<{ secret: string; otpauthUrl: string }>>('/candidate/2fa/generate');
  return data.data;
};

export const verify2FAToken = async (token: string): Promise<{ verified: boolean; backupCodes?: string[] }> => {
  const { data } = await api.post<ApiEnvelope<{ verified: boolean; backupCodes?: string[] }>>('/candidate/2fa/verify', { token });
  return data.data;
};

export const disable2FA = async (): Promise<{ success: boolean }> => {
  const { data } = await api.post<ApiEnvelope<{ success: boolean }>>('/candidate/2fa/disable');
  return data.data;
};
// ─── Applied Job IDs ──────────────────────────────────────────────────────────
...
export const getJobPreferences = async (): Promise<JobPreferencesPayload> => {
  const { data } = await api.get<ApiEnvelope<{ jobPreferences: JobPreferencesPayload }>>('/candidate/preferences/job');
...
export const updateJobPreferences = async (payload: JobPreferencesPayload): Promise<JobPreferencesPayload> => {
  const { data } = await api.put<ApiEnvelope<{ jobPreferences: JobPreferencesPayload }>>('/candidate/preferences/job', payload);
  return data.data.jobPreferences;
};
